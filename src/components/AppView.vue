<script setup lang="ts">
// ---------------------------------------------------------------------------
// AppView — Invoice editor shell (sidebar + preview + toast)
// Layer: components (depends on: composables, components)
// ---------------------------------------------------------------------------
// Initializes all composables, provides them via typed injection keys,
// and renders the sidebar, preview panel (with action bar), and toast
// notifications. Reads entry mode from App.vue to determine initial
// behavior (new, continue, upload).
//
// M1+ wiring:
// - useInvoice: invoice state, resetInvoice, loadInvoice
// - useJsonIO: JSON export/import
// - useLogoUpload: logo base64 conversion
// - useToast: notification system
// - useHistory: invoice history stack (for M3)
// - useTemplate: active template selection
// ---------------------------------------------------------------------------

import { inject, provide, onMounted, ref, watch, type Ref } from 'vue'

import SidebarShell from '@/components/sidebar/SidebarShell.vue'
import PreviewPanel from '@/components/preview/PreviewPanel.vue'
import Toast from '@/components/shared/Toast.vue'

import { useInvoice } from '@/composables/useInvoice'
import { useJsonIO } from '@/composables/useJsonIO'
import { useLogoUpload } from '@/composables/useLogoUpload'
import { useToast } from '@/composables/useToast'
import { useHistory } from '@/composables/useHistory'
import { useTemplate } from '@/composables/useTemplate'

import {
  INVOICE_KEY,
  JSON_IO_KEY,
  LOGO_UPLOAD_KEY,
  TOAST_KEY,
  HISTORY_KEY,
  TEMPLATE_KEY,
  ENTRY_MODE_KEY,
  SIDEBAR_TAB_KEY,
  type EntryMode,
  type SidebarTab,
} from '@/composables/injection-keys'

// Provide sidebar tab ref so SidebarShell can react to tab changes
const sidebarTab: Ref<SidebarTab> = ref('editor')
provide(SIDEBAR_TAB_KEY, sidebarTab)

// Inject entry mode from App.vue
const entryMode = inject<Ref<EntryMode>>(ENTRY_MODE_KEY)

// Initialize composables
const invoice = useInvoice()
const jsonIO = useJsonIO()
const logoUpload = useLogoUpload()
const toast = useToast()
const history = useHistory()
const template = useTemplate()

// Provide via typed injection keys
provide(INVOICE_KEY, invoice)
provide(JSON_IO_KEY, jsonIO)
provide(LOGO_UPLOAD_KEY, logoUpload)
provide(TOAST_KEY, toast)
provide(HISTORY_KEY, history)
provide(TEMPLATE_KEY, template)

// Sync template changes to invoice data so JSON export includes active template
watch(
  () => template.activeTemplate.value,
  (newTemplate) => {
    invoice.invoice.value.template = newTemplate
  },
)

// Handle entry mode on mount
onMounted(() => {
  if (!entryMode?.value) return

  const mode = entryMode.value

  switch (mode.type) {
    case 'new':
      // Fresh invoice is the default — nothing extra to do
      break

    case 'continue':
      // Switch sidebar to history tab
      // (history is already populated from localStorage by useHistory on init)
      sidebarTab.value = 'history'
      break

    case 'upload':
      if (mode.payload) {
        // Load the uploaded invoice data
        const loadedTemplate = invoice.loadInvoice(mode.payload)
        template.setTemplate(loadedTemplate)
        // Add to in-memory history
        history.addToHistory(mode.payload)
      }
      break
  }

  // Consume the entry mode to prevent re-triggering on HMR re-mounts
  entryMode.value = null
})
</script>

<template>
  <div class="app-view">
    <SidebarShell />
    <PreviewPanel />
    <Toast />
  </div>
</template>

<style scoped>
/* display: contents makes this wrapper transparent to the #app grid layout,
   so SidebarShell and PreviewPanel become direct grid children */
.app-view {
  display: contents;
}
</style>
