...
Define the root layout structure of the app: the two-column grid with a fixed-width sidebar and fluid preview panel, the sidebar brand header, tab bar, sticky action bar, and the preview panel topbar and background.
...
#### Scenario: Sidebar has dark mangrove background
- **WHEN** the sidebar renders
- **THEN** it SHALL have mangrove dark-green background with a strong right shadow

### Requirement: Sidebar brand header

The sidebar brand header SHALL render a hexagon icon (`⬡`) in a 34px × 34px coral background box with shadow (`box-shadow: 0 3px 14px rgba(232,115,74,.45)`) alongside the serif "Billed" heading. The subtitle SHALL use mono 9px with 2.5px letter-spacing, uppercase, in `var(--color-text-dim)`.
...
#### Scenario: Inactive tabs dim
- **WHEN** a tab is inactive
- **THEN** it SHALL use dim text color
...
The print.css SHALL hide `.preview-topbar` alongside `.sidebar`, `.template-switcher`, `.action-bar`, and `.no-print` during print. The preview panel SHALL expand to full width with white background.
